...
import * as styles from './koncepthotels.scss';
import { MapsModalData } from './koncepthotels.hook';
import { useTranslation } from 'react-i18next';

/**
 * Renders KonceptHotelsModal
 */
const KonceptHotelsModal = () => {
  const dispatch = useDispatch();
  const { t } = useTranslation();
  const { about, stars, achievments, awards, distination } = MapsModalData();
  return (
    <>
      ...
      <div className={styles.mapsmodal}>
        <img src={`img/KonceptHotels.png  `} className={styles.modelImg} />
        <div className={styles.hotelDescription}>
          <h1 className={styles.hotelDescriptionHeading}>
            {t(
              'hotel-concepts.about-maps-modals-data.maps-modal1.koncept-hotels.title'
            )}
          </h1>
          <p className={styles.hotelDescriptionPara}>
            {t(
              'hotel-concepts.about-maps-modals-data.maps-modal1.koncept-hotels.discription'
            )}
          </p>
          <p className={styles.hotelDescriptionHashTag}>
            {t(
              'hotel-concepts.about-maps-modals-data.maps-modal1.koncept-hotels.hash-tag'
            )}
          </p>
      ...
      <div className={styles.aboutHotel}>
        <div className={styles.aboutHotelDesign}>
          <p className={styles.aboutHotelDetailsLabal}>
            {t(
              'hotel-concepts.about-maps-modals-data.maps-modal1.koncept-hotels.about.achievments.title'
            )}
          </p>
        ...
        <div className={styles.hotelDescriptionList}>
          <ul className={styles.hotelDescriptionListUl}>
            <h2>
              {t(
                'hotel-concepts.about-maps-modals-data.maps-modal1.koncept-hotels.about.awards.title'
              )}
            </h2>
            {awards.map((item, id) => (
              <li key={id}>
      ...
      <div className={styles.aboutHotelDistination}>
        <p className={styles.aboutHotelDetailsLabal}>
          {t(
            'hotel-concepts.about-maps-modals-data.maps-modal1.koncept-hotels.about.distination.title'
          )}
        </p>
